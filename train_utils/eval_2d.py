...
def eval_burgers(model,
    ...
    for x, y in pbar:
        ...
        data_loss = myloss(out, y)  # l2相对误差

        loss_u, f_loss = PINO_loss(out, x[:, 0, :, 0], v)  # MSE误差
        test_err.append(data_loss.item())
        f_err.append(f_loss.item())

    mean_f_err = np.mean(f_err)
    # std_f_err = np.std(f_err, ddof=1) / np.sqrt(len(f_err)) #标准差/长度
    std_f_err = np.std(f_err, ddof=1)  # 标准差

    mean_err = np.mean(test_err)
    # std_err = np.std(test_err, ddof=1) / np.sqrt(len(test_err))
    std_err = np.std(test_err, ddof=1)

    print(f'==Averaged relative L2 error mean: {mean_err}, std error: {std_err}==\n'
          f'==Averaged equation MSE error mean: {mean_f_err}, std error: {std_f_err}==')
